    ...
    >
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        <header className="mb-10">
          <div className="mb-4">
            <span className="inline-block px-4 py-1.5 bg-teal-100 text-teal-700 text-sm font-medium rounded-full">
              {post.category}
            </span>
          </div>
          ...
          {/* Meta Info */}
          <div className="flex flex-wrap gap-4 text-sm text-gray-600 pb-8 border-b border-gray-200">
        ...
        {post.tags && post.tags.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-16 pb-8 border-b border-gray-200">
            {post.tags.map((tag) => (
              <Link
                key={tag}
                href={`/${locale}/blog?tag=${tag}`}
                className="px-4 py-2 bg-gray-100 text-gray-700 rounded-full text-sm font-medium hover:bg-teal-100 hover:text-teal-700 transition-colors"
              >
                #{tag}
              </Link>
            ))}
              ...
              {relatedPosts.map((related: any) => {
                ...
                return (
                  <Link
                    ...
                    className="group block p-6 bg-white border border-gray-200 rounded-xl hover:shadow-xl hover:border-teal-500 transition-all duration-300"
                  >
                    {related.coverImage && (
                      <img
                    ...
                    <h3 className="font-bold text-lg mb-2 text-gray-900 group-hover:text-teal-600 transition-colors">

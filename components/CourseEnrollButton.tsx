...
export default function CourseEnrollButton({ course }: { course: Course }) {
  ...
  const handleClose = () => {
    ...
    setStep('method');
    setMethod('');
    setForm({ name: '', email: '', phone: '', txnId: '' });
  };
  ...
  return (
    <>
      <button
        ...
        className="bg-emerald-700 text-white px-6 py-3 rounded-xl font-bold hover:bg-emerald-800 transition-all flex items-center gap-2 text-sm"
      >
        Enroll Now
      </button>

      {isOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-stone-900/80 backdrop-blur-sm" onClick={(e) => e.target === e.currentTarget && handleClose()}>
          <div className="bg-white rounded-3xl max-w-md w-full p-8 relative shadow-2xl max-h-[90vh] overflow-y-auto">
            <button onClick={handleClose} className="absolute top-6 right-6 text-stone-400 hover:text-stone-900">
              <X className="w-6 h-6" />
            </button>
            ...
            {step === 'method' && (
              <>
                ...
                <p className="text-stone-500 text-sm mb-6">
                  Enrolling in: <strong>{course.title}</strong> · PKR {course.price.toLocaleString()}/month
                </p>
              ...
              <>
                ...
                <div className="bg-stone-50 rounded-2xl p-5 border border-stone-200 mb-6">
                  <p className="text-sm font-bold text-stone-500 uppercase tracking-widest mb-1">
                    Send PKR {course.price.toLocaleString()} to:
                  </p>
                  <p className="font-mono text-emerald-700 font-bold break-all">
                    {selectedMethod.account || 'PayFast checkout link will be provided'}
                  </p>
                ...
                <div className="flex gap-3 mt-6">
                  ...
                  <button
                    ...
                    className="flex-[2] py-3 rounded-xl bg-emerald-700 text-white font-bold hover:bg-emerald-800 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
            ...
            {step === 'done' && (
              <div className="text-center py-6">
                ...
                <p className="text-stone-500 mb-6">
                  We received your enrollment for <strong>{course.title}</strong>.{' '}
                  Our team will verify your payment and send course access within{' '}
                  <strong>24 hours</strong> to your email.
                </p>
                <p className="text-sm text-stone-400 mb-8">
                  Questions? Call:{' '}
                  <a href="tel:+923441722419" className="text-emerald-700 font-bold">
                    +92 344 1722419
                  </a>
                </p>
                <button
                  onClick={handleClose}
                  className="w-full bg-stone-900 text-white py-3 rounded-xl font-bold hover:bg-stone-800"
                >
                  Close
                </button>
              </div>
